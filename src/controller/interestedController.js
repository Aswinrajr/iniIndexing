...
// Collection names
const interestedCollectionName = 'interested';
    ...
    const interestedProperties = await Promise.all(querySnapshot.docs.map(async doc => {
      ...
      if (propertyId) {
        ...
        if (propertyDocSnap.exists()) {
          ...
          return { id: doc.id, ...docData, propertyDetails: propertyData,createdAt: date, formattedDate };
        }
      }
...
export const updatePropertyStatusWithComments = async (propertyId, status, comment, dateTimeArray) => {
  try {
    ...
    const docRef = firestoreDoc(firestore, 'interested', propertyId);
